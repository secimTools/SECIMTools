...
def main():
    ...
    if args.ngroup != None:
        ...
        df_rank = df.apply(lambda x: pd.qcut(x.rank(method='min'), q=groupBins, labels = False, duplicates = 'drop')+1, axis=0)
        df_rank.insert(0, uniId, dat.wide.index)
    else:
        df_rank = df.apply(lambda x: x.rank(method='min'), axis = 0)
        df_rank.insert(0, uniId, dat.wide.index)
