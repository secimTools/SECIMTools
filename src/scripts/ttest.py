...
def getOptions(myopts=None):
    ...
    standard.add_argument(
        ...
        required=True,
        default=False,
        help="Choice of the test: has to be either paired or unpaired.",
    )
    standard.add_argument(
        "-o",
        "--order",
...
def main(args):
    ...
    if args.pairing == "unpaired":
        ...
        # Extracting data from the interface. BUG. Only extract data colu;mns.
        data_frame = dat.transpose()
        exit(f"DEBUG: {data_frame}")
        # Extracting number of features. This will depend on whether the user has provided
        ...
        #exit(f"DEBUG: {data_frame.iloc[:,0:-6].head()}")
        # DEBUG, remove non-numeric columns
        ...
        for j in range(0, number_of_features):
            ...
            # We also transpose the df here so it will be easier to use.
            ...
            # BUG
            #logger.info(f"DEBUG: {data_frame_manipulate_transpose.head()}")
        ...
        # Running pairwise unpaired (two-sample) t-test for all pairs of group levels that are
        # saved in groups_pairwise.
        for i in range(0, number_of_groups_pairwise):
            # Extracting the pieces of the data frame that belong to groups saved in the i-th
            # unique pair.
    ...
    if args.pairing == "paired":
        order = args.order
        logger.info("Perform a pairwise t-test based on a pairing variable: {order}.".format(order))
        ...
        # pairid those samples are removed with a warning. If pairdid corresponds to exactly two
        # samples (which is correct) but groupid-s are NOT different those values will be also
        # removed.
