...
def getOptions():
    ...
    group2.add_argument('--order', dest='order', action='store', required=False, default=False, help='Name of the column with the runOrder')
...
def pltBoxplot(dat, ax):
    ...
    #Changin run order
    if dat.anno:
        runOrder =  dict(dict(dat.design[dat.anno])[dat.anno[0]]).items()
        runOrder = [sampleID for sampleID,runOrder in sorted(runOrder, key=lambda runOrder:runOrder[1])]
        samp = dat.wide[dat.sampleIDs]
        samp=samp[runOrder]
    else:
        samp = dat.wide[dat.sampleIDs]
...
def main(args):
    # Import data
    logger.info(u'Importing data with following parameters: \n\tWide: {0}\n\tDesign: {1}\n\tUnique ID: {2}\n\tGroup Column: {3}\n\tRun Order: {4}'.format(args.fname, args.dname, args.uniqID, args.group, args.order))
    if args.order:
